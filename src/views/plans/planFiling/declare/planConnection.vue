  ...
  <div id="planConnection">
    ...
    <el-table :data="dataList">
      ...
      <el-table-column prop="emergencyTypeFullPath" label="事件类别" :show-overflow-tooltip="true" width="180">
        <template slot-scope="scope">
          <el-tag style="margin-right: 10px"
            ...
            <span>/<dict-tag style="display: inline-block" :showType="2" :options="getOptions(item[0])"
                             :value="item[1]"/></span>
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="planSystemFullPath" label="行政级别" :show-overflow-tooltip="true" width="180">
        <template slot-scope="scope">
          <dict-tag :options="(type==='1'?dict.type['26']:dict.type['525'])"
...
<script>
import {connectionList, connection, getRecordInfo, enConnectionList, enRecordAuditInfo} from "@/api/plans/planRecord";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      (this.type == 1 ? connectionList : enConnectionList)(this.queryParams).then(response => {
    ...
    getInfo() {
      (this.type == 1 ? getRecordInfo : enRecordAuditInfo)(this.recordId).then((res) => {
        if (res.data.connectionPlanIds) {
          this.ids = res.data.connectionPlanIds.split(',')
        }
      })
    },
    /** 搜索按钮操作 */
    ...
    linkUpHandle(row) {
      ...
      this.$modal.confirm('是否确认衔接当前预案？').then(function () {
        let planIds = _this.ids
        planIds.push(row.recordId.toString())
        let data = {
          recordId: _this.recordId,
          connectionPlanIds: planIds.join(',')
        }
    ...
    cancelLinkUpHandle(row) {
      ...
      this.$modal.confirm('是否取消衔接当前预案？').then(function () {
        let planIds = _this.ids.filter(item => item != row.recordId)
        let data = {
          recordId: _this.recordId,
          connectionPlanIds: planIds.join(',')
        }
     ...
     * 获取查询的事件类别
     */
    getEmergencyType(val) {
      this.queryParams.emergencyTypeList = []
      val.forEach(item => {
        this.queryParams.emergencyTypeList.push(item.join(","))
      })
    }
