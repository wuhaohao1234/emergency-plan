      ...
      <el-table-column label="操作" align="center" width="180" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <div style="display: inline-block;margin-right: 10px">
            <download-file :fileName="scope.row.fileName" :fileKey="scope.row.fileKey" :buttonDesc="2"
                           :size="'mini'"></download-file>
          </div>
          <el-button
            v-if="handleType===2"
            size="mini"
...
import {getRecordInfo, editRecord} from "@/api/plans/planRecord";
...
export default {
  ...
  methods: {
    ...
    conciseManual() {
      ...
      this.$emit('conciseManual', str)
    },
    //应急文书
    emergencyDoc() {
      let str = JSON.stringify(this.uploadList)
      this.$emit('emergencyDoc', str)
    }
  }
};
</script>
