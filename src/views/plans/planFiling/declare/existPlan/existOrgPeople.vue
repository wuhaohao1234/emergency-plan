...
<template>
  <div id="orgPeople">
    <el-dialog title="组织成员" :visible.sync="visible" width="70%"
      ...
      <el-button style="float: right;margin-bottom: 10px" v-if="handleType===2"
                 icon="el-icon-plus" @click="addPeople" size="mini"
                 type="primary">添加人员
      </el-button>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$emit('addOrgPeople', this.userList)
    },
    //选择负责人
    selectLeader(row) {
      this.userList.forEach(item => {
