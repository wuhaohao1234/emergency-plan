<template>
  <div class="app-container" id="intelligentAssistance">
    <div v-if="!documentVisible&&!detailVisible">
...
<!--        </el-form-item>-->
      ...
      <el-table :data="dataList"  v-loading="loading">
        ...
        <el-table-column heade-align="center" prop="emergencyTypeFullPath" label="事件类别" :show-overflow-tooltip="true"
          ...
          <template slot-scope="scope">
            <el-tag style="margin-right: 10px" v-for="(item,index) in buildTypeLabel(scope.row.emergencyTypeFullPath)"
                    :key="index">
        ...
        <el-table-column align="center" prop="planSystemFullPath" label="行政级别" :show-overflow-tooltip="true"
                         width="120">
          <template slot-scope="scope">
        ...
        <el-table-column heade-align="center" label="操作" align="center" width="160"
                         class-name="small-padding fixed-width"  fixed="right">
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="text"
              @click="handleDetail(scope.row)">预案详情
            </el-button>
            <el-button
              v-if="type==='1'"
              size="mini"
              type="text"
              @click="handleDocument(scope.row)">快速文书
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <plan-express-document ref="planExpressDocument" v-if="documentVisible"
                           @goBack="()=>this.documentVisible = false"></plan-express-document>
    <el-dialog
      class="structuralDialog"
      :show-close="false"
      :visible.sync="detailVisible"
      :fullscreen="true"
      :before-close="()=>{this.detailVisible=false}">
      <plan-structural-detail ref="planStructuralDetail" @goBack="()=>{this.detailVisible=false}"/>
    </el-dialog>
  </div>
</template>
...
import {findByPlanId} from "@/api/plans/planRecord";
import {getDicts} from "@/api/system/dict/data";
import PlanExpressDocument from "./planExpressDocument";
import PlanStructuralDetail from "./../plans/planFiling/audit/planStructuralDetail";

export default {
  name: "intelligentAssistance",
  dicts: ['4', '26', '525', '601', '608', '611', '622','662'],
  components: {PlanExpressDocument, PlanStructuralDetail},
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    async handleDetail(row) {
      await findByPlanId(row.planId).then(res => {
        if (res.code == 200 && res.data) {
          this.detailVisible = true;
          let style={height:'100vh'}
          this.$nextTick(() => {
            this.$refs.planStructuralDetail.init(row, res.data,style);
          });
        } else {
          this.$message.warning('当前预案未备案，无法查看详情')
        }
      })
    },
    /**
    ...
    handleDocument(row) {
      ...
      this.$nextTick(() => {
        this.$refs.planExpressDocument.init(row)
      })
    },
    goBackHandle() {
